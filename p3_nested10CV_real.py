...
from sklearn.model_selection import permutation_test_score, GridSearchCV, ParameterGrid, cross_val_score, cross_val_predict
from sklearn.metrics import matthews_corrcoef, confusion_matrix, roc_curve, auc, make_scorer, roc_auc_score, precision_recall_curve, precision_score
...
from lightgbm.sklearn import LGBMModel, LGBMClassifier, LightGBMError
...
import time, os, json, re, copy, glob, threading, multiprocessing, sys, itertools
...
def select_adjusted_threshold(y, y_proba):
	...
	gmean_thr_highest_mcc = geometric_mean_score(y, tup_thr_with_highest_mcc[2])

	return thr_with_highest_mcc, highest_mcc, pre_thr_highest_mcc, rec_thr_highest_mcc, f1_thr_highest_mcc, accu_thr_highest_mcc, gmean_thr_highest_mcc

########################################################################

def cross_val_predictProba_10cv_paramsEvaluation(clf, p_grid, X, y, sample_weight, cv, l_mean_prediction_time, l_mcc, l_auc, l_avgpre, l_pre, l_rec, l_f1, l_accu, l_gmean, l_aucpr, threshold, l_thr = None):
	...
	for p in ParameterGrid(p_grid):
		...
		if threshold:
			thr_p, mcc_p, pre_p, rec_p, f1_p, accu_p, gmean_p = select_adjusted_threshold(y, y_pred_proba_params[str(p)][1].drop(['ind'], axis = 1).values)
			l_thr.append(thr_p)
			...
			gmean_p = geometric_mean_score(y, y_pred_proba_params[str(p)][0].drop(['ind'], axis = 1).values)
...
def nested_cross_validation(args):
	...
	if oversamp:
		if oversamp == "SMOTE":
			sm = SMOTE(k_neighbors = 5, sampling_strategy = 0.5, n_jobs = 1)

		elif oversamp == "ADASYN":
			sm = ADASYN(n_neighbors = 5, sampling_strategy = 0.5, n_jobs = 1)

		elif oversamp == "ROS":
			sm = RandomOverSampler(sampling_strategy = 0.5)

		elif oversamp == "TOMEKLINK":
			sm = TomekLinks(sampling_strategy = "majority", n_jobs = 1)

		elif oversamp == "SMOTEENN":
			sm = SMOTEENN(sampling_strategy = 0.5, smote = SMOTE(sampling_strategy = 0.5, k_neighbors = 5, n_jobs = 1), enn = EditedNearestNeighbours(sampling_strategy = "majority", n_neighbors = 5, n_jobs = 1))
	...
	if clf == 'CART':
		...
		base_clf = RandomForestClassifier(n_jobs = 1, oob_score = True, n_estimators = 1000, verbose = 0, random_state = seed)
		...
		base_clf = XGBClassifier(n_estimators = 1000, subsample = 0.8, colsample_bytree = 0.8, silent = True, n_jobs = 1, random_state = seed)
		...
		base_clf = LGBMClassifier(n_jobs = 1, n_estimators = 1000, subsample = 0.8, colsample_bytree = 0.8, random_state = seed)
		...
		base_clf = LogisticRegression(n_jobs = 1, random_state = seed, dual = False, class_weight = None)
		...
		base_clf = SVC(kernel='linear', random_state = seed, probability=True)
		...
		base_clf = SVC(kernel='rbf', random_state = seed, probability=True)

		directory = "hyperparameter_tuning_SVCR" + '_'.join(p_grid.keys()) + '_' + date
		...
		base_clf = KNeighborsClassifier(n_neighbors = 5, n_jobs = 1)
		...
		base_clf = Pipeline([("selectfeat", selectobject), ("model", RandomForestClassifier(n_estimators = 1000, oob_score = True, n_jobs = 1, verbose = 0, random_state = seed))])
		p_grid['selectfeat__k'] = range(2, int(X_outer_train.shape[0]/2) + 1)
		...
		base_clf = Pipeline([("selectfeat", selectobject), ("model", LGBMClassifier(n_estimators = 1000, subsample = 0.8, colsample_bytree = 0.8, n_jobs = 1, random_state = seed))])
		p_grid['selectfeat__k'] = range(2, int(X_outer_train.shape[0]/2) + 1)
		...
		base_clf = Pipeline([("selectfeat", selectobject), ("model", LogisticRegression(n_jobs = 1, random_state = seed, dual = False, class_weight = None))])
		p_grid['selectfeat__k'] = range(2, int(X_outer_train.shape[0]/2) + 1)
		...
		base_clf = Pipeline([("selectfeat", selectobject), ("model", SVC(kernel='linear', random_state = seed, probability=True))])
		p_grid['selectfeat__k'] = range(2, int(X_outer_train.shape[0]/2) + 1)
		...
		base_clf = Pipeline([("selectfeat", selectobject), ("model", SVC(kernel='rbf', random_state = seed, probability=True))])
		p_grid['selectfeat__k'] = range(2, int(X_outer_train.shape[0]/2) + 1)
		directory = "hyperparameter_tuning_FeatureSelection_SVCR_" + '_'.join([s.replace(s[0:s.find('__') + 2], '') for s in sorted(p_grid.keys())]) + '_' + date
	...
	cross_val_predictProba_10cv_paramsEvaluation(clf_to_tune, p_grid_tuning, X_outer_train, y_outer_train, sample_weight, inner_cv, l_mean_prediction_time, l_inner_mcc, l_inner_auc, l_inner_avgpre, l_inner_pre, l_inner_rec, l_inner_f1, l_inner_accu, l_inner_gmean, l_inner_aucpr, threshold, l_inner_thresholds)
	print(time.strftime("%Y-%m-%d at %H:%M:%S"))
	...
	dict_inner_loop['inner_GMEAN'] = l_inner_gmean
	...
	df_inner_loop['inner_GMEAN'] = df_inner_loop['inner_GMEAN'].apply(lambda x: round(x, 3) if type(x) is float else x)
	...
	df_inner_loop['rank_test_GMEAN'] = df_inner_loop['inner_GMEAN'].rank(ascending = False, method = 'min', na_option = 'bottom').astype(int)
	...
	# if clf.upper() == 'PCART':
	# 	dt_name = "common_sel_hyperparameter_tuning_FeatureSelection_CART_" + "seed" + str(seed) + '_' + date
	# 	tree_filename = directory + '/' + dt_name + "_tree_PCART" + '.svg'
	# 	if isinstance(best_estimator, Pipeline):
	# 		if 'selectfeat' in best_estimator.named_steps.keys():
	# 			kept_best_feature_indices = best_estimator.named_steps['selectfeat'].get_support(indices = True)
	# 			kept_best_feature_names = [l_feat[idx] for idx, _ in enumerate(l_feat) if idx in kept_best_feature_indices]
	# 			data_omc = data.loc[:, kept_best_feature_names]
	# 			x_omc = data_omc.values
	# 			drawdtreeviz(tree_filename, best_estimator, x_omc, y, target_name = target_name, feature_names = kept_best_feature_names, l_class_names = l_class_names)
	# else: pass
...
def calculate_random_seed_metrics(y, y_outer_test_preds, y_outer_test_probas, set_inner_selected_features, list_inner_best_params, list_inner_best_thresholds, list_inner_best_scores, base_clf, seed, directory, metric, p_grid):
	...
	final_gmean = geometric_mean_score(y, y_outer_test_preds)
	...
	outer_results['final_GMEAN'] = final_gmean
	...
	outer_results['final_GMEAN'] = outer_results['final_GMEAN'].apply(lambda x: round(x, 3) if type(x) is float else x)
	...
	df_confusion_matrix = pd.DataFrame.from_records([{"random_seed":seed, "tn":tn, "fp":fp, "fn":fn, "tp":tp, "MCC":final_mcc, "AUC":final_auc, "AUCPR":final_aucpr, "AVGPRE":final_avgpre, "PRE":final_pre, "REC":final_rec, "F1":final_f1, "ACCU":final_accu, "GMEAN":final_gmean}])
	...
	df_confusion_matrix['GMEAN'] = df_confusion_matrix['GMEAN'].apply(lambda x: round(x, 3) if type(x) is float else x)
...
def deployment_phase(p_grid, sample_weight, seed, base_clf, X, y, inner_cv, directory, metric, target_name, l_feat, l_class_names, data, threshold, smp):
	...
	cross_val_predictProba_10cv_paramsEvaluation(clf_depl, p_grid_depl, X, y, sample_weight, inner_cv, l_prediction_time_depl, l_depl_mcc, l_depl_auc, l_depl_avgpre, l_depl_pre, l_depl_rec, l_depl_f1, l_depl_accu, l_depl_gmean, l_depl_aucpr, threshold, l_depl_thr)
	print(time.strftime("%Y-%m-%d at %H:%M:%S"))
	...
	df_depl['GMEAN'] = df_depl['GMEAN'].apply(lambda x: round(x, 3) if type(x) is float else x)
	...
	df_depl['rank_test_GMEAN'] = df_depl['GMEAN'].rank(ascending = False, method = 'min', na_option = 'bottom').astype(int)
	...
	### Decision Tree graphviz
	# if clf.upper() == 'CART':
	# 	dt_name = "hyperparameter_tuning_CART_" + "seed" + str(seed) + '_' + date
	# 	tree_filename = directory + '/' + dt_name + "tree_CART_DeploymentPhase_" + '.svg'
	# 	drawdtreeviz(tree_filename, best_estimator_depl, X, y, target_name = target_name, feature_names = l_feat, l_class_names = l_class_names)

	# elif clf.upper() == 'PCART':
	# 	dt_name = "hyperparameter_tuning_FeatureSelection_CART_" + "seed" + str(seed) + '_' + date
	# 	tree_filename = directory + '/' + dt_name + "tree_CART_DeploymentPhase_" + '.svg'
	# 	if isinstance(best_estimator_depl, Pipeline):
	# 		if 'selectfeat' in best_estimator_depl.named_steps.keys():
	# 			kept_feat_indices = best_estimator_depl.named_steps['selectfeat'].get_support(indices = True)
	# 			kept_feat_names = [l_feat[idx] for idx, _ in enumerate(l_feat) if idx in kept_feat_indices]
	# 			data_omc = data.loc[:,kept_feat_names]
	# 			x_omc = data_omc.values
	# 			drawdtreeviz(tree_filename, best_estimator_depl, x_omc, y, target_name = target_name, feature_names = kept_feat_names, l_class_names = l_class_names)
	# else: pass
...
def default_model_performance(base_clf, X, y, sample_weight, outer_cv, seed, p_grid, smp, target_name, feature_names, l_class_names):
	"""
...
#################################################################
	...
	gmean_default = geometric_mean_score(y, y_pred_default)
	...
	dict_default['GMEAN'] = gmean_default
	...
	df_default['GMEAN'] = df_default['GMEAN'].apply(lambda x: round(x, 3) if type(x) is float else x)
	...
	df_default_confusion_matrix = pd.DataFrame.from_records([{"random_seed":seed, "tn":tn, "fp":fp, "fn":fn, "tp":tp, "MCC":mcc_default, "AUC":auc_default, "AUCPR":aucpr_default, "AVGPRE":avgpre_default, "PRE":pre_default, "REC":rec_default, "F1": f1_default, "ACCU": accu_default, "GMEAN": gmean_default}])
	...
	df_default_confusion_matrix['GMEAN'] = df_default_confusion_matrix['GMEAN'].apply(lambda x: round(x, 3) if type(x) is float else x)
...
def permutation_test_10cv(base_clf, X, y, sample_weight, seed, smp):
	...
	permutation_gmean = geometric_mean_score(y, y_pred)

	df_permutation = pd.DataFrame.from_records([{'random_seed':seed,'params':'Permutated 10CV','MCC':permutation_mcc,'AUC':permutation_auc,'AUCPR':permutation_aucpr,'AVGPRE':permutation_avgpre,'PRE':permutation_pre,'REC':permutation_rec, 'F1':permutation_f1, 'ACCU':permutation_accu, 'GMEAN':permutation_gmean}])
	...
	df_permutation['GMEAN'] = df_permutation['GMEAN'].apply(lambda x: round(x, 3) if type(x) is float else x)
...
if __name__=='__main__':
	...
	if clf not in ['CART','RF','XGB', 'LGBM', 'LR', 'SVC', 'SVCR', 'KNN', 'PCART', 'PRF', 'PXGB', 'PLGBM', 'PLR', 'PSVC', 'PSVCR']:
	...
	if hparams:
		if (clf == 'CART') | (clf == 'PCART'):
			hparams = [{'model_max_depth':[5,None],'model__min_samples_leaf': [1,2], 'model__max_features': ['auto','log2',None]}]
		elif (clf == 'RF') | (clf == 'PRF'):
			hparams = [{'model__n_estimators':[100,250,500,1000, 1500], 'model__min_samples_leaf': [1,2,3], 'model__max_features':['auto','log2',None]}]
		elif (clf == 'XGB') | (clf == 'PXGB'):
			hparams = hparams = [{'model__n_estimators':[100,250,500,1000,1500],'model__learning_rate':[0.05,0.1,0.2,0.3],'model__min_child_weight':[1,2,3],'model__max_depth':[4,5,6,7],'model__scale_pos_weight':[1,2,3],'model__gamma':[0.05,0.1,0.2,0.3]}]
		elif (clf == 'LGBM') | (clf == 'PLXGB'):
			hparams = [{'model__n_estimators':[100,250,500,1000,1500],'model__num_leaves':[20,50,100],'model__max_depth':[1,2,3,4,5,8],'model__learning_rate':[0.025,0.05,0.1,0.2]}]
		elif (clf == 'LR') | (clf == 'PLR'):
			hparams = [{'model__solver':['newton-cg','lbfgs','liblinear'],'model__penalty':['l2'],'model__C':[10.0,1.0,0.1,0.01]}]
		elif (clf == 'SVC') | (clf == 'PSVC') | (clf == 'SVCR') | (clf == 'PSVCR'):
			hparams = [{'model__C':[0.001,0.01,0.1,1.0,10.0],'model__kernel':['linear','poly','rbf','sigmoid'],'model__gamma':['scale','auto']}]
	...
	metric = metric.upper()
	if metric not in ['MCC','AUC','AVGPRE','PRE','REC','F1','AUCPR','ACCU', 'GMEAN']:
	...
	if clf == 'CART':
		...
		log_filename = "hyperparameter_tuning_SVCR_" + date + ".log"
		tuned_algo = "SVCR"
		default_tsv = "default_SVCR_predictive_performance.tsv"
		permutation_tsv = "permutation_test_SVCR_predictive_performance.tsv"
		omc_tsv = "omc_SVCR_linear_predictive_performance.tsv"
		...
		log_filename = "hyperparameter_tuning_FeatureSelection_SVCR_" + date + ".log"
		tuned_algo = "SVCR completed with OMC"
		default_tsv = "default_SVCR_predictive_performance.tsv"
		permutation_tsv = "permutation_test_SVCR_predictive_performance.tsv"
		omc_tsv = "omc_SVCR_linear_predictive_performance.tsv"
    ...
    # Class names
	if 'Patient ID_Aliquot ID' in data.columns.tolist():
		class_names = np.array(list(set(data['Drug response'].values)))
	elif 'Aliquot ID' in data.columns.tolist():
		class_names = np.array(list(set(data['Drug response'].values)))
	else:
		class_names = np.array(list(set(data['response'].values)))
	...
	for seed in range(len(l_random_seeds)):
		for pgrid in hparams:
			...
			default_res_seed = default_model_performance(base_clf_from_nestedcv_step, X, y, sample_weight, StratifiedKFold(n_splits = 10), l_random_seeds[seed], p_grid_from_nestedcv_step, sampler_from_nestedcv_step, target_name, feature_names, l_class_names)
	...
	if clf == 'CART':
		...
		plot_filename_aucpr = "hyperparameter_tuning_RandomForest_AUCPR_metric.png"
		...
		plot_filename_aucpr = "hyperparameter_tuning_XGBoost_AUCPR_metric.png"
		...
		plot_filename_aucpr = "hyperparameter_tuning_LightGBM_AUCPR_metric.png"
		...
		plot_filename_aucpr = "hyperparameter_tuning_FeatureSelection_LogisticRegression_AUCPR_metric.png"
		...
		plot_filename_aucpr = "hyperparameter_tuning_FeatureSelection_KNN_AUCPR_metric.png"
		...
		plot_filename_aucpr = "hyperparameter_tuning_FeatureSelection_SVC_linear_AUCPR_metric.png"

	elif clf == 'SVCR':
		plot_filename_mcc = "hyperparameter_tuning_FeatureSelection_SVCR_linear_MCC_metric.png"
		plot_filename_auc = "hyperparameter_tuning_FeatureSelection_SVCR_linear_AUC_metric.png"
		plot_filename_aucpr = "hyperparameter_tuning_FeatureSelection_SVCR_linear_AUCPR_metric.png"
		...
		plot_filename_aucpr = "hyperparameter_tuning_FeatureSelection_RandomForest_AUCPR_metric.png"
		...
		plot_filename_aucpr = "hyperparameter_tuning_FeatureSelection_XGBoost_AUCPR_metric.png"
		...
		plot_filename_aucpr = "hyperparameter_tuning_FeatureSelection_LightGBM_AUCPR_metric.png"
